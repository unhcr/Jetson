...
def make_baseline(current_month, lag, admin_level='admin1'):
    ...
    hm= true[['true']].unstack(level=admin_unit).rolling(window=12, center=False).mean().stack(dropna=False)
    ...
    #################################
    ## Export
    true.to_csv(f"ml/output_data/{admin_level}_lag{lag}/results_baseline.csv")
