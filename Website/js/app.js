...
function drawLinesByMonth(month_in, year_in) {
	...
	if (month === month_in && year === year_in) {
	    var previous_coord = {lat: data_line[10], lng: data_line[11]};
	    var current_coord = {lat: data_line[5], lng: data_line[6]};
    ...
    console.log('Drawing ' + polylines.length + ' lines.')
...
function drawHeatmapByMonth(month_in, year_in) {
	...
	if (month === month_in && year === year_in) {
	    var current_coord = {lat: data_line[2], lng: data_line[3]};
